...
class AuthManager {
    ...
    /**
     * For the first time login, access_token scope only have the "idToken"
     * Here we are setting the subscribe role to the user and change the accessToken scopes
     *
     *
     */
    static setScopesAndRoles() {
        const idToken = Utils.getCookie('USER_ID_TOKEN', 'DEFAULT');
        const body = JSON.parse(Buffer.from(idToken.split('.')[1], 'base64'));
        const token = Utils.getCookie('USER_ACCESS_TOKEN', 'DEFAULT');
        if (!token || !idToken) {
            console.log('Redirect to Anonymous View');
            return true;
        } else {
            ...
            if (body.tpp_associated !== 'true') {
                ...
                return promisedResponse
                    ...
                    .then((data) => {
                        if (data.scopeUpdated) {
                            return true;
                        } else {
                            console.error('Error in webApp update call');
                            return false;
                        }
                    });
            }
            console.error('User does not have the TPP access: ' + JSON.stringify(body));
            return false;
        }
    }
    ...
    static getUserFromToken() {
        ...
        // check whether the OBIE integration is enabled
        if (Settings.app.OBIE_integration) {
